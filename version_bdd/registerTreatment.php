...
if(!empty($_POST)) {
  ...
  // Sinon on tente d'enregistrer l'utilisateur en base de données
  else {
    // On vérifie si un utilsiateur enregistré en base de donnée porte déjà ce nom
    $checkUser = getUserByName($_POST["user_name"], $db);
    if(empty($checkUser)) {
    ...
    // Si on a trouvé un utilisateur portant déjà ce nom on renvoie au formulaire
    else {
